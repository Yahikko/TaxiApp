...
public class PassengerMapsActivity extends FragmentActivity implements OnMapReadyCallback {
            ...
            public void onGeoQueryReady() {
                if (!isDriverFound && searchRadius < 100) {
                    ...
                    Toast.makeText(PassengerMapsActivity.this,
                            "Now searching radius is " + searchRadius,
        ...
        if (requestCode == CHECK_SETTINGS_CODE) {
            switch (resultCode) {
                ...
                case Activity.RESULT_OK:
                    Log.d("MainActivity", "user has agreed to change" +
                            "location settings");
                    startLocationUpdates();
                    break;

                case Activity.RESULT_CANCELED:
                    Log.d("MainActivity", "user has NOT agreed to change" +
                            "location settings");
                    isLocationUpdatesActive = false;
                    updateLocationUi();
                    break;
    ...
    protected void onResume() {
        ...
        if (isLocationUpdatesActive && checkLocationsPermissions()) {

            startLocationUpdates();
        } else if (!checkLocationsPermissions()) {
